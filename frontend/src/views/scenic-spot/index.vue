...
const form = reactive({ name: '', cityId: '', address: '', phone: '', status: 'open' })
...
const columns = [
  ...
  { title: '开放状态', slotName: 'status', width: 110 },
...
const fetchData = async () => {
  ...
  try {
    const [res, c] = await Promise.all([
      request.get('/scenic-spots', { params: { search: searchKey.value, page: pagination.current, pageSize: pagination.pageSize } }),
      request.get('/cities')
    ])
...
const handleBatchDelete = async () => {
  ...
  try {
    await request.delete('/scenic-spots/bulk', { data: { ids: selectedKeys.value } })
    Message.success('批量删除成功')
    selectedKeys.value = []
    fetchData()
  } catch (err) {
    // 错误消息已由 request 拦截器显示
  }
}
...
const handleModalOk = async () => {
  ...
  try {
    if (isEdit.value) await request.put(`/scenic-spots/${currentId.value}`, form)
    else await request.post('/scenic-spots', form)
