    ...
    <a-card title="景点管理">
      ...
      >
        ...
        <template #status="{ record }">
          <a-tag size="small" :color="record.status === 'open' ? 'green' : 'gray'">
            {{ record.status === 'open' ? '开放' : '闭园' }}
          </a-tag>
        </template>
...
const handleModalOk = async () => {
  ...
  try {
    if (isEdit.value) {
      await request.put(`/scenic-spots/${currentId.value}`, form)
    } else {
      await request.post('/scenic-spots', form)
    }
